...
Gait involving subconscious posture reflex can be guided using sensory illusion so that user is not aware of such illusion. In fact, many previous research using illusionary sensory feedback to the user realizes subconscious gait guidance. However, approximately two-steps latency as attitude reflection response after sudden sensory input may affect accurate gait guidance in real time. For example, it is too late to give sensory feedback to let user turn to the left when start to turn to the right. Thus, it explicitly requires prediction of gait movement two-steps ahead of the current gaiting in real time. This paper suggests that gait prediction method using multilayered perceptron as predictor for gait motion data sets including five kinds of gait motion such as straight ahead, right turn and left turn. Evaluation result shows that proposed method achieved gait prediction one step ahead as numerical experiment. 
